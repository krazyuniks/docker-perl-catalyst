...
RUN apt -y update && \
    apt -y dist-upgrade && \
    apt -y install build-essential libtool libpng-dev libgif-dev libjpeg-dev libtiff-dev libfreetype6-dev libssl-dev libexpat-dev libzip-dev libgd-dev libxml2-dev libdb-dev libyaml-dev libxslt-dev libcurl4-openssl-dev libmcrypt-dev libpcre2-dev zlib1g-dev libjson-c-dev libgmp-dev libmariadb-dev-compat libmariadb-dev curl && \
